...
- [x] Read docs/REFERENCE_NOTES.md end to end before touching code. Key decision recorded there (2026-07-01): both fork candidates are closed-source, so we build our own shell from scratch, mirroring TalkToMeV's structure — Script subclass, single backend seam, provider classes for state. Do not decompile or copy code from the closed mods.
